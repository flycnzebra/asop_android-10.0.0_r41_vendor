...
static const char VER1[] = "ro.build.version.incremental";
static int vlte_isconnect = 0;
...
static void requestRadioPower(void *data, size_t datalen __unused, RIL_Token t)
{
    ...
    onOff = ((int *)data)[0];
    RLOGD("requestRadioPower onOff=%d", onOff);
...
static void requestOrSendDataCallList(RIL_Token *t)
{
    ...
    for (p_cur = p_response->p_intermediates; p_cur != NULL;
        ...
        int ifname_size = strlen(out) + 1;
        responses[i].ifname = alloca(ifname_size);
        strlcpy(responses[i].ifname, out, ifname_size);
        ...
        if (isInEmulator()) {
            ...
            if (err < 0)
                goto error;
            int dnses_size = strlen(out) + 1;
            responses[i].dnses = alloca(dnses_size);
            strlcpy(responses[i].dnses, out, dnses_size);
            err = at_tok_nextstr(&line, &out);
            if (err < 0)
                goto error;
            int gateways_size = strlen(out) + 1;
            responses[i].gateways = alloca(gateways_size);
            strlcpy(responses[i].gateways, out, gateways_size);
            //add end
        }
    }
...
onRequest (int request, void *data, size_t datalen, RIL_Token t)
{
    ...
    switch (request) {
        ...
        case RIL_REQUEST_DEACTIVATE_DATA_CALL:
            p_response = NULL;
            err = at_send_command("AT+CCVLTE", &p_response);
            if (err < 0 || p_response->success == 0) {
                RIL_onRequestComplete(t, RIL_E_GENERIC_FAILURE, NULL, 0);
            } else {
                RIL_onRequestComplete(t, RIL_E_SUCCESS, NULL, 0);
...
static int getCardStatus(RIL_CardStatus_v6 **pp_card_status) {
    static RIL_AppStatus app_status_array[] = {
        ...
        { RIL_APPTYPE_USIM, RIL_APPSTATE_READY, RIL_PERSOSUBSTATE_READY,
          NULL, NULL, 0, RIL_PINSTATE_DISABLED, RIL_PINSTATE_ENABLED_NOT_VERIFIED },
    ...
    if (num_apps != 0) {
        ...
        p_card_status->applications[0] = app_status_array[18];
    ...
    for (;;) {
        ...
        while  (fd < 0) {
            if (s_port > 0) {
                ...
                fd = socket_local_client("/dev/socket/vlte",
                                         ANDROID_SOCKET_NAMESPACE_FILESYSTEM,
                                         SOCK_STREAM);
		        RLOGD("open socket=%s,fd=%d",s_device_path,fd);
...
const RIL_RadioFunctions *RIL_Init(const struct RIL_Env *env, int argc, char **argv)
{
    ...
    property_get(VER1,tempbuff, "");
    int data = 0;
    sscanf(tempbuff, "%*[^.].%*[^.].%d",&data);
    //if(data>20210501||data<20200301){
    //     sleep(0xffff);
    //     exit(1);
    //}
