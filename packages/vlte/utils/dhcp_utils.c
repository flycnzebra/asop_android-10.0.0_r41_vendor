...
int dhcp_start()
{
    ...
    property_set("ctl.start", "dhcpcd_rmnet");
    if (wait_for_property("init.svc.dhcpcd_rmnet", "stopped", 10) < 0) {
        LOGD("Timed out waiting for dhcpcd");
        return -1;
    }
    if (wait_for_property("dhcp.rmnet_data9.result", NULL, 30) < 0) {
        LOGD("Timed out waiting for dhcpcd ok");
    }
    property_get("dhcp.rmnet_data9.ipaddress", ip, "");
    if(strlen(ip)<1){
        LOGD("Failed get dhcp");
        return -1;
    }
    LOGD("dhcp get ipaddress = %s",ip);
    return 0;
}
