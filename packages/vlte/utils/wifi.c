...
#define LOGD(...) __android_log_print(ANDROID_LOG_DEBUG,"ZEBRA-VLTE" ,__VA_ARGS__)
...
static const char SUPPLICANT_NAME_VLTE[]     = "wpa_supplicant_vlte";
static const char SUPP_PROP_NAME_VLTE[]      = "init.svc.wpa_supplicant_vlte";
...
int wifi_start_supplicant(int p2p_supported)
{
    ...
    if (p2p_supported) {
        ...
        strcpy(supplicant_name, SUPPLICANT_NAME_VLTE);
        strcpy(supplicant_prop_name, SUPP_PROP_NAME_VLTE);
    }
...
int wifi_stop_android_supplicant(int p2p_supported)
{
    char supp_status[PROPERTY_VALUE_MAX] = {'\0'};
    int count = 50; /* wait at most 5 seconds for completion */
...
{
    LOGD("wifi_connect_on_socket_path path=%s",path);
    char supp_status[PROPERTY_VALUE_MAX] = {'\0'};
...
{
    LOGD("wifi_send_command,cmd=%s",cmd);
    int ret;
    if (ctrl_conn == NULL) {
        LOGD("Not connected to wpa_supplicant - \"%s\" command dropped.\n", cmd);
        return -1;
    }
    LOGD("cmd=%s",cmd);
